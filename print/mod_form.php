...
class mod_print_mod_form extends moodleform_mod {
    function definition() {
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'contentsection', get_string('contentheader', 'print'));
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'print'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'print'), array('size'=>3));
            if (count($options) > 1) {
        ...
        $mform->addElement('advcheckbox', 'printheading', get_string('printheading', 'print'));
        $mform->setDefault('printheading', $config->printheading);
        $mform->addElement('advcheckbox', 'printintro', get_string('printintro', 'print'));
        $mform->setDefault('printintro', $config->printintro);
        $mform->addElement('advcheckbox', 'printlastmodified', get_string('printlastmodified', 'print'));
        $mform->setDefault('printlastmodified', $config->printlastmodified);

        // add legacy files flag only if used
        if (isset($this->current->legacyfiles) and $this->current->legacyfiles != RESOURCELIB_LEGACYFILES_NO) {
            $options = array(RESOURCELIB_LEGACYFILES_DONE   => get_string('legacyfilesdone', 'print'),
                             RESOURCELIB_LEGACYFILES_ACTIVE => get_string('legacyfilesactive', 'print'));
            $mform->addElement('select', 'legacyfiles', get_string('legacyfiles', 'print'), $options);
